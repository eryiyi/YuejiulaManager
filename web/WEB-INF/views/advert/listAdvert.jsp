      ...
      <div class="box-content">
        ...
        <table class="table">
          ...
          <c:forEach items="${list}" var="e" varStatus="st">
            <tr>
              <td>${st.index + 1}</td>
              <td><a href="javascript:void (0);"><img class="img-rounded" width="50" height="50" src="${e.adPic}" onclick="ImgShow(event)" ></a></td>
              <td>${e.schoolName}</td>
              <td>
              ...
              <td>
                ...
                <button class="btn btn-primary" type="button" onclick="deleteAdvert('${e.adId}')">删除</button>
              </td>
            </tr>
          </c:forEach>
          </tbody>
...
<script type="text/javascript">
  function searchIndex(e){
    ...
    if(_index <= ${page.pageCount} && _index >= 1){
        window.location.href="#module=ajax/listAdvert&page="+_index+"&size="+size;
    }else{
  ...
  function nextPage(_page) {
    ...
    if ((page <= ${page.pageCount} && page >= 1)) {
        window.location.href="#module=ajax/listAdvert&page="+page+"&size="+size+"&schoolId="+schoolId;
    } else {
      alert("请输入1-${page.pageCount}的页码数");
    }
  }
  ...
  function deleteAdvert(_id){
    ...
    $.ajax({
      ...
      success: function(_data) {
        ...
        if(data.success){
          alert("删除成功");
            window.location.href="#module=ajax/listAdvert&page="+${page.page}+"&_t="+new Date().getTime();
        }else{
          var _case = {1:"删除失败"};
